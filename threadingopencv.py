...
# Remove the bounding boxes with low confidence using non-maxima suppression
    ...
    def write_record_to_DB(self,time_now,direction):
        names=['Muskan Nehal','Sagar Shah', 'Karan Nayak','Naira Rohida','Hemant Kulkarni','Rhea Haran','Sneha Rohra', 'Arjun Punjabi','Ayu Talreja','Mohan Katyar']
        mycursor=mydb.cursor()
        insert_stmt = (
        "INSERT INTO identity_status(userName, floor, door, department,time, direction)"
        "VALUES (%s, %s, %s, %s, %s, %s)"
                    )
        data = (random.choice(names), self.floor, self.door, 'Stocks',time_now, direction)
    ...
    def validate_up_count(self,cap,current_frame,count,time_now):
        ...
        end_time=time_now
        sql = "SELECT * FROM identity_status where floor=" +str(self.floor)+" and door="+str(self.door)+" and direction='in' and time BETWEEN '"+ str(start_time) + "' AND '"+ str(end_time) + "'"
        mycursor = mydb.cursor()
        ...
        if len(myresult)!=count:
            ...
            file_name="G:\\Counting-People-master\\entry_floor_"+str(self.floor)+"time_"+str(timestamp)+".jpg"
            ...
            sql="Select distinct a.* from (Select b.* From (SELECT * FROM identity_status where floor="+str(self.floor)+" and door="+str(self.door)+"  and time < '"+ str(time_now)+"' Order by time desc LIMIT 1 ) b UNION select * from identity_status where floor="+str(self.floor)+" and door="+str(self.door)+" and direction='in' and time between '"+str(time_now)+"' and '"+str(time_diff)+"')a "
            mycursor = mydb.cursor()
            mycursor.execute(sql)
            myresult = mycursor.fetchall()
            mycursor.close()
    ...
    def validate_down_count(self,cap,current_frame,count,time_now):
        ...
        while seconds!=3:
            ...
            sql = "SELECT * FROM identity_status where floor="+str(self.floor)+" and door="+str(self.door)+" and direction='out' and time LIKE '%"+str(current_time)+"%'"
            mycursor = mydb.cursor()
            mycursor.execute(sql)
            myresult = mycursor.fetchall()  
            seconds=seconds+1
        ...
        if len(myresult)!=count:
            ...
            print("[INFO] tailgating happened at  ",videotime)
            timestamp = time_now.strftime("%Y%m%d_%H-%M-%S")
            file_name="G:\\Counting-People-master\\exit_floor_"+str(self.floor)+"time_"+str(timestamp)+".jpg"
            if not cv.imwrite(file_name, current_frame):
                raise Exception("Could not write image")
            duration = 1000  # milliseconds
            freq = 440  # Hz
            winsound.Beep(freq, duration)
            time_diff=time_now - datetime.timedelta(seconds=5)
            sql="Select distinct a.* from (Select b.* From (SELECT * FROM identity_status where floor="+str(self.floor)+" and door="+str(self.door)+"  and time < '"+ str(time_now)+"' Order by time desc LIMIT 1 ) b UNION select * from identity_status where floor="+str(self.floor)+" and door="+str(self.door)+" and direction='out' and time between '"+str(time_now)+"' and '"+str(time_diff)+"')a "
            mycursor = mydb.cursor()
            mycursor.execute(sql)
            myresult = mycursor.fetchall()
            mycursor.close()
...
def main():
    thread1 = myThread(1,'TownCentre_Trim.mp4',1)
    thread2 = myThread(2,'TownCentre_Trim1.mp4',2)
    thread1.start()
    thread2.start()
